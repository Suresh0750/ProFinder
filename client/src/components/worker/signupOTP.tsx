...
const SignupOTP  = ({userId}:{userId:string}) => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const result = await CustomerOtp({otpValue:otpString,role:'worker',userId}) 
      console.log(result)
  ...
  const handleResendOTP = async () => {
    ...
    try {
      if(timer!=0) return //* prevent multiple click

      
      const result = await CustomerResend({userId,role:'worker'})
      console.log(result)
      if(result?.data?.success){
        toast.success(result.data.message)
        setTimer(60);
